...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 18:15:42 2023
...
class like():
    ...
    def likelihood(self):
        self.LL = self.f(self.T) + sum([np.log(self.lamda(x)) + self.f(x) for x in xVec]) + sum([np.log(psi) - self.f(y) for y in yVec])
        ...
        while t<nRep: 
            ...
            if temp<math.exp(propTheta.lnr): #accept
                ...
                with open('chain.csv', 'a', newline='') as file1:
                    ...
                    writer1 = csv.writer(file1)
                    # Write the column names
                    writer1.writerow([propTheta.beta])
                t+=1
                acc+=1
